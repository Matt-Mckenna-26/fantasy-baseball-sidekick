...
const configSchema = z
  .object({
    ...
    /**
     * Optional Exa web-search API key. When set, the co-manager gains a read-only
     * `web_search` tool that grounds current facts (a player's present MLB team, this
     * week's sleepers, breaking news) in live web results instead of stale training data.
     * Omitted -> the tool is simply not offered. Never logged; a secret like the others.
     */
    exaApiKey: z.string().min(1).optional(),
  })
  .refine(
    (c) =>
      c.chatProvider !== 'azure' ||
      (Boolean(c.azureOpenAiEndpoint) &&
        Boolean(c.azureOpenAiApiKey) &&
        Boolean(c.azureOpenAiDeployment)),
    {
      message:
